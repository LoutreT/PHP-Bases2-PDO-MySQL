...
"En voici deux qu'on utilise souvent: "concat" et "count".
Il y en a de nombreuses autres concat."

"Elle permet de concaténer des colonnes pour recevoir un tableau plus facile à
traiter ensuite en PHP. Par exemple, tu peux faire ceci:"
...
Tu veux savoir combien il y a de villes dans le tableau?
Tu peux simplement demander via SQL en utilisant la fonction" COUNT().

SELECT COUNT(*) FROM météo;

_______________________________________________________________________________

Tu veux vérifier qu'il n'y a pas eu une double entrée pour Bruxelles?
Utilise COUNT() et une clause WHERE
pour savoir le nombre de fois que "Bruxelles" se trouve dans la table météo.
...
--
  ...
  Réponse 1 = SELECT COUNT(*) FROM octocats WHERE gender='M';---------> 32
  ...
  Réponse 4 = SELECT COUNT(*) FROM octocats WHERE firstname='Nadia';-------> 2
...
--  que l'année de la colonne 'birthdate' et affiche le prénom de tous les
--  octocts et leur année de naissance.
--  (indice: comment dit-on 'année' en anglais?)"
